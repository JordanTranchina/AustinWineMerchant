document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyFiltersAndSort() {
        ...
        let result = originalInventory.filter(item => {
            const matchesSearch = (
                normalizeText(item.brand).includes(searchTerm) ||
                normalizeText(item.maguey).includes(searchTerm) ||
                normalizeText(item.description).includes(searchTerm) ||
                normalizeText(item.price).includes(searchTerm) ||
                normalizeText(item.alcohol).includes(searchTerm)
            );
